...
class ThreadGetRequest(BaseContentGenerateRequest):
    thread_id: int = Field(description="The id of the thread to get")
...
async def create_thread_and_generate_content(request: ThreadCreateRequest):
    ...
    topic, model, extra_instructions = (
        request.topic,
        request.model,
...
async def generate_content(request: ThreadGetRequest):
    thread_id, model, extra_instructions = (
        request.thread_id,
        request.model,
        request.extra_instructions,
    )
    ...
    async def stream_content():
        async for content in ts.generate_content(
            ThreadGenerate(
