...
class Thread:
    def __init__(self, topic: str, id: int, previous_concepts: list[str] = []) -> None:
        """
...
class ThreadService:
    ...
    async def __create_thread(self, topic: str) -> int:
        ...
        self.threads[thread_id] = Thread(topic=topic, id=thread_id)
        return thread_id

    async def __load_thread(self, thread_id: int) -> Thread:
        """
        Load a thread from the database asynchronously.
        """
        topic, contents = await load_thread_with_topics(thread_id)
