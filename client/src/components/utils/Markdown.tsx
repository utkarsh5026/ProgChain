      ...
      <ReactMarkdown
        ...
        components={{
          ...
          ul: ({ node, ...props }) => (
            <ul
              {...props}
              className="my-6 ml-6 list-disc space-y-2 text-zinc-300"
            />
          ),
          ol: ({ node, ...props }) => (
            <ol
            ...
            return (
              <Card className="my-6 overflow-x-auto bg-zinc-900 border-zinc-200 border-2 relative group">
                <button
                  ...
                  className="absolute right-2 top-2 px-2 py-1 text-xs rounded bg-zinc-800 text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-zinc-700"
                >
                  Copy
                </button>
                <pre
          ...
          code: ({ node, inline, ...props }) =>
            inline ? (
              <code
                {...props}
                className="relative rounded-md2bg-zinc-800 px-[0.3rem] py-[0.2rem] font-mono text-sm text-primary border-2 border-zinc-700"
              />
            ) : (
